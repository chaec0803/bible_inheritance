...
const mocks = vi.hoisted(() => ({
  ...
  legacyObjects: [{ object_key: 'gifts/friend/gift-1/one' }] as Array<Record<string, unknown>>,
  batch: vi.fn(),
  r2Head: vi.fn(),
  r2Get: vi.fn(),
  r2Delete: vi.fn(),
  ...
  getD1: () => ({
    prepare: (sql: string) => ({
      bind: () => ({
        ...
        all: async () => ({ results: sql.includes('source_recording_id IS NULL') ? mocks.legacyObjects : mocks.recordings }),
      }),
    }),
    batch: mocks.batch,
  }),
...
describe('받은 선물 접근·삭제 회귀', () => {
  beforeEach(() => {
    ...
    mocks.legacyObjects = [{ object_key: 'gifts/friend/gift-1/one' }];
  ...
  it('새 정책의 선물을 삭제해도 발신자의 원본 녹음은 삭제하지 않는다', async () => {
    mocks.legacyObjects = [];
    const response = await DELETE(new Request('https://example.test/api/gifts/gift-1', { method: 'DELETE' }), giftContext);
    expect(response.status).toBe(200);
