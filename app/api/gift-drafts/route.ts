...
export async function POST(request: Request) {
  ...
  for (const recipientUserId of recipientUserIds) {
    const [a, b] = canonicalFriendPair(user.id, recipientUserId);
    ...
    const block = await db.prepare(`SELECT id FROM friend_blocks
      ...
      LIMIT 1`)
      .bind(user.id, recipientUserId, recipientUserId, user.id)
      .first<{ id: string }>();
    if (block) return Response.json({ error: '차단 관계인 친구가 포함되어 있어 선물을 만들 수 없습니다.' }, { status: 403 });
  }
  let journey;
