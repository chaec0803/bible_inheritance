...
export function GiftLetterComposer({ value, disabled, onChange }: { value: GiftLetterInput; disabled?: boolean; onChange: (letter: GiftLetterInput) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setSeconds(elapsed);
      if (elapsed >= GIFT_LETTER_AUDIO_MAX_SECONDS) void recordingSessionRef.current?.stop();
    }, 250);
    return () => window.clearInterval(timer);
  }, [recording]);

  const selectType = (type: GiftLetterInput['type']) => {
    if (recording) recordingSessionRef.current?.dispose();
    recordingSessionRef.current = null;
    setRecording(false);
    setError('');
    if (previewUrl) URL.revokeObjectURL(previewUrl);
    ...
    try {
      ...
      const session = await createRecordingSession({
        ...
        onCaptured: async (capture) => {
          ...
          setRecording(false);
          if (!capture?.blob.size) return setError('음성 편지가 녹음되지 않았어요. 다시 시도해 주세요.');
          const durationSeconds = Math.max(1, Math.min(GIFT_LETTER_AUDIO_MAX_SECONDS, Math.round(capture.durationMs / 1_000)));
          if (capture.blob.size > 5 * 1024 * 1024) return setError('음성 편지가 너무 커요. 90초 안으로 다시 녹음해 주세요.');
          if (previewUrl) URL.revokeObjectURL(previewUrl);
          setPreviewUrl(URL.createObjectURL(capture.blob));
          setVoiceCandidate({ type: 'voice', dataUrl: await blobToDataUrl(capture.blob), mimeType: capture.mimeType, sizeBytes: capture.blob.size, durationSeconds });
        },
        onError: () => {
          recordingSessionRef.current = null;
          setRecording(false);
          setError('녹음 중 문제가 생겼어요. 다시 시도해 주세요.');
        },
      });
  ...
  return <section className="gift-letter-composer" aria-labelledby="gift-letter-title">
    ...
    {recording && <div className="gift-letter-recording"><span><Mic size={18} /> 녹음 중 · {seconds}초 / {GIFT_LETTER_AUDIO_MAX_SECONDS}초</span><button type="button" onClick={() => void recordingSessionRef.current?.stop()}><Pause size={16} /> 녹음 마치기</button></div>}
    ...
    {mode === 'voice' && previewUrl && <div className="gift-letter-preview"><strong>{value.type === 'voice' ? '음성 편지를 확정했어요' : '음성 편지가 준비됐어요'}</strong>{/* oxlint-disable-next-line jsx-a11y/media-has-caption -- 사용자가 방금 녹음한 음성 편지에는 별도 자막 파일이 없습니다. */}<audio controls onError={(event) => reportAudioElementFailure(event.currentTarget)} src={previewUrl} /><div><button type="button" disabled={disabled} onClick={() => void startVoice()}><RotateCcw size={15} /> 재녹음</button>{voiceCandidate && value.type !== 'voice' && <button type="button" disabled={disabled} onClick={() => onChange(voiceCandidate)}><Play size={15} /> 음성 편지 확정</button>}</div><span><Play size={14} /> 재생해 본 뒤 확정해 주세요.</span></div>}
